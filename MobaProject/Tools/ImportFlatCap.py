...
ART = "/Game/Moba/Art"
SRC_FBX = Path(r"C:\Users\Thedo\Downloads\flat-cap\source\unpacked\Flathat_Sketchfab\Flathat.fbx")
SRC_TEX = Path(r"C:\Users\Thedo\Downloads\flat-cap\source\unpacked\Flathat_Sketchfab\textures")
LOG_PATH = Path(r"C:\Users\Thedo\Documents\MobaRepo\MobaProject\Saved\ImportFlatCap.log")
lines = []
...
def create_material(color, roughness):
    mat_path = ART + "/M_FlatCap"
    if unreal.EditorAssetLibrary.does_asset_exist(mat_path):
        unreal.EditorAssetLibrary.delete_asset(mat_path)
    factory = unreal.MaterialFactoryNew()
    mat = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
        "M_FlatCap", ART, unreal.Material, factory
    )
    if not mat:
        log("Failed to create M_FlatCap")
        return None
...
def main():
    log("Importing flat cap into {}".format(ART))
    unreal.EditorAssetLibrary.make_directory(ART)

    pngs = [
        (SRC_TEX / "FlatHat_Basecolor.png", "T_FlatCap_Color"),
        (SRC_TEX / "FlatHat_Roughness.png", "T_FlatCap_Roughness"),
    ]
    ...
    if SRC_FBX.is_file():
        import_file(SRC_FBX, "SM_FlatCap", make_fbx_options())
    else:
    ...
    roughness = set_texture(
        ART + "/T_FlatCap_Roughness",
        unreal.TextureCompressionSettings.TC_MASKS,
        False,
    )
    mat = create_material(color, roughness)

    mesh_path = ART + "/SM_FlatCap"
    if not unreal.EditorAssetLibrary.does_asset_exist(mesh_path):
        for alt in (ART + "/Flathat", ART + "/FlatHat", ART + "/FlatCap"):
